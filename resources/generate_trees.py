...
class Palette(NamedTuple):
    ...
    def add_blocks(self, wood: str, log_paths: dict[Pos, Pos], leaf_paths: dict[Pos, int], seasonality: str):
        """
        ...
        :param seasonality: The seasonality block state for the leaf blocks
        """
        ...
        for leaf_pos, dist in leaf_paths.items():
            block = create_leaf_block_tag(wood, dist, seasonality)
...
def main():
    ...
    for name, tree in TREES.items():
        ...
        for seasonality in ('evergreen', 'deciduous', 'monsoonal'):
            leaves[name] = make_tree(name, seasonality, tree.normal)
            make_tree(name, seasonality, tree.large, '_large')
...
def make_tree(wood: str, seasonality: str, tree: str | None, suffix: str = '') -> int:
    ...
    :param seasonality: The seasonality blockstate of the tree leaves
    ...
    if tree_type == RANDOM:
        for n in range(1, 1 + arg):
            src = '%s%d' % (tree, n)
            dest = '%s%s_%s/%d.nbt' % (wood, suffix, seasonality, n)
            try:
                base, leaves = make_single_structure(wood, src, seasonality)
        ...
        try:
            base, overlay, leaves = make_overlay_tree(wood, tree, seasonality)
        ...
        for layer in range(1, 1 + len(arg)):
            for n in range(1, 1 + arg[layer - 1]):
                ...
                try:
                    base, _ = make_single_structure(wood, src, seasonality)
                    save_structure(dest, base)
                except AssertionError as e:
                    print('Error %s.nbt -> %s.nbt : %s' % (src, dest, e))
...
def make_single_structure(wood: str, name: str, seasonality: str) -> tuple[RootTag, int]:
    ...
    palette.add_blocks(wood, log_paths, leaf_paths, seasonality)
...
def make_overlay_tree(wood: str, tree: str, seasonality: str) -> tuple[RootTag, RootTag, int]:
    ...
    overlay_palette.add_blocks(wood, translate(overlay_log_paths, -offset), translate(overlay_leaf_paths, -offset), seasonality)
...
def create_leaf_block_tag(wood: str, distance: int, seasonality: str) -> CompoundTag:
    ...
    block['Properties']['leaf_seasonality'] = StringTag(seasonality)
    return block
